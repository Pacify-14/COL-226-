...
module Term = struct
  ...
  (* Height of a term *)
  let ht t =
    let rec ht_acc max_h work = match work with
      | [] -> max_h
      | (d, t) :: rest -> match t with
        | V _ -> ht_acc (max max_h d) rest
        | Node (_, childs) ->
            let new_work = List.map (fun c -> (d + 1, c)) (Array.to_list childs) in
            ht_acc (max max_h d) (new_work @ rest)
    in
    ht_acc 0 [(0, t)]

  (* Size (# of nodes) of a term *)
  let size t =
    let rec size_acc acc work = match work with
      | [] -> acc
      | t :: rest -> match t with
        | V _ -> size_acc (acc + 1) rest
        | Node (_, childs) -> size_acc (acc + 1) (Array.to_list childs @ rest)
    in
    size_acc 0 [t]
