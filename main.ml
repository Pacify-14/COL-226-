...
(* Convert Ast.expr Matrix to float list list *)
...
let rec calculate_determinant matrix =
  ...
  | _ ->
      ...
      |> List.fold_left (+.) 0.0
(* Convert Ast.expr Matrix to float list list *)
let matrix_to_float (rows : Ast.expr list list) : float list list =
  List.map (fun row ->
    List.map (fun elem ->
      match elem with
...
let minor matrix i j =
  let without_row = List.filteri (fun idx _ -> idx <> i) matrix in
  let without_col = List.map (fun row -> List.filteri (fun idx _ -> idx <> j) row) without_row in
  calculate_determinant without_col

(* Compute matrix of minors *)
let minor_matrix matrix =
  List.mapi (fun i row ->
    List.mapi (fun j _ -> minor matrix i j) row
  ) matrix

(* Apply cofactor signs *)
let cofactor_matrix minor_matrix =
  List.mapi (fun i row ->
    List.mapi (fun j x -> if (i + j) mod 2 = 0 then x else -.x) row
  ) minor_matrix

(* Multiply matrix by scalar *)
let scalar_mult_matrix scalar matrix =
...
let rec step (e : Ast.expr) : Ast.expr =
  ...
  | Ast.Inv e ->
    if not (is_val e) then Ast.Inv (step e)
    else (
      match e with
      | Ast.Matrix rows ->
          if not (is_square_matrix rows) then
            failwith "Matrix must be square for inversion"
          else
            let float_matrix = matrix_to_float rows in
            let det = calculate_determinant float_matrix in
            if det = 0.0 then
              failwith "Matrix is singular (determinant = 0)"
            else
              let minors = minor_matrix float_matrix in
              let cofactors = cofactor_matrix minors in
              let adjugate = transpose cofactors in
              let inv_float = scalar_mult_matrix (1.0 /. det) adjugate in
              (* Convert back to Ast.expr Matrix *)
              let inv_expr =
                List.map (fun row ->
                  List.map (fun x -> Ast.Float x) row
                ) inv_float
              in
              Ast.Matrix inv_expr
      | _ -> failwith "inv_mat requires a matrix"
    ) 
  | Ast.Dim2 e ->
...
| Ast.Lt, Ast.Vector v1, Ast.Vector v2 ->
  ...
  | Ast.Eq, Ast.Int a, Ast.Float b -> Ast.Bool (float_of_int a = b)
| Ast.Eq, Ast.Float a, Ast.Int b -> Ast.Bool (a = float_of_int b)
| Ast.Ne, Ast.Int a, Ast.Float b -> Ast.Bool (float_of_int a <> b)
| Ast.Ne, Ast.Float a, Ast.Int b -> Ast.Bool (a <> float_of_int b)
  ...
  | Ast.Ne, Ast.Int a, Ast.Int b -> Ast.Bool (a <> b)
  | Ast.Ne, Ast.Float a, Ast.Float b -> Ast.Bool (a <> b)
